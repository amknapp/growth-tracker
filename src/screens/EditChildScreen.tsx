...
interface Props {
  navigation: NativeStackNavigationProp<RootStackParamList, 'EditChild'>;
  route: RouteProp<RootStackParamList, 'EditChild'>;
}

const EditChildScreen: React.FC<Props> = ({ navigation, route }) => {
  const { childId } = route.params;
  const getChild = useAppDataStore(state => state.getChild);
  const updateChild = useAppDataStore(state => state.updateChild);
  ...
  const showAvatarOptions = () => {
    if (avatarUri) {
      // Show remove option if avatar exists
